...
const AddAccount: React.FC<AddAccountProps> = ({ onClose, onAccountAdded }) => {
  const [sourceName, setSourceName] = useState('');
  const [type, setType] = useState<'checking' | 'savings' | 'debit_card'>('checking');
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
        <div className="flex justify-between items-start mb-4">
          <h2 className="text-xl font-semibold text-gray-800">Aggiungi Nuovo Conto</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <InputField
              ...
              placeholder="Inserisci il nome della fonte (es. Conto Intesa)"
              required
            />
          </div>
