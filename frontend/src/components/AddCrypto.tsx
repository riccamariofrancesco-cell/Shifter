...
const AddCrypto: React.FC<AddCryptoProps> = ({ onClose, onCryptoAdded }) => {
  const [sourceName, setSourceName] = useState('');
  const [type, setType] = useState<'BTC' | 'ETH' | 'LTC' | 'BCH' | 'XRP' | 'ADA' | 'DOT' | 'LINK'>('BTC');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Failed to create cryptocurrency');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
        <div className="flex justify-between items-start mb-4">
          <h2 className="text-xl font-semibold text-gray-800">Aggiungi Nuova Criptovaluta</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <InputField
              ...
              placeholder="Inserisci il nome della fonte (es. Wallet Bitcoin)"
              required
            />
          </div>
          ...
          <div className="flex justify-end space-x-3">
            ...
            >
              {loading ? 'Creazione...' : 'Crea Criptovaluta'}
            </button>
          </div>
        </form>
      </div>
